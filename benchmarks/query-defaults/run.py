...
parser=argparse.ArgumentParser(description='Passthrough query replay; outputs are observations, not relevance scores.')
...
if stage=='initial':
 ...
 (p/'metadata.json').write_text(json.dumps({'revision':revision,'tracked_diff_sha256':hashlib.sha256(diff).hexdigest(),'dataset_sha256':hashlib.sha256((root/'benchmarks/codex-search-2026-09-11/queries.json').read_bytes()).hexdigest(),'source_identity_note':'checkout revision and tracked diff; establish executable provenance by building in this worktree before running','binary':b,'sha256':hashlib.sha256(pathlib.Path(b).read_bytes()).hexdigest(),'version':subprocess.check_output([b,'--version'],text=True).strip(),'comparison':'passthrough only; portable mode removed; exact historical query/limit flags; live uncontrolled providers; diagnostics enabled','started':datetime.datetime.now(datetime.timezone.utc).isoformat()},indent=2))
...
for i,job in enumerate(jobs):
 modes=['passthrough']
 for mode in modes:
  label=mode+'-'+job['id']; args=[b]+job['args']
  ...
  record={'id':label,'args':args,'started_utc':timestamp,'wall_ms':(time.monotonic()-start)*1000,'exit_code':code,'outer_timeout':code==124,'finished_utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'stdout':stdout,'stderr':stderr}
