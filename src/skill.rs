...
- Bing serializes query spaces as `%20` and literal plus signs as `%2B`, preserving the decoded query. Requests send `q` and optional region `cc`; browser AJAX/session parameters and `pq` are not added. This encoding alignment is not a demonstrated relevance fix.
...
- CLI searches that exclude Bing and Yahoo skip their impersonated transports and root-store initialization. Initialization remains outside the search budget. Bing defaults to a fixed Chrome/macOS impersonated transport validated for Bing response fidelity; library callers can immediately roll Bing back to reqwest with `TransportOptions { bing_transport: BingTransport::Standard, ..Default::default() }`. This does not change Yahoo or other providers. `KestrelClient::with_engines_and_parser_capacity(engines, n)` constructs Bing and Yahoo impersonated transports only when selected; later searches for an omitted provider return `KestrelError::InvalidRequest` before requests. Clones preserve this policy. Existing constructors retain both transports.
...
- Within a provider attempt, both HTTP backends retry HTTP 408/429/5xx unless a non-2xx bot challenge is detected. Bing's default impersonated transport and Yahoo retry all send errors; the standard backend retries only timeout/connect/request errors. Successful-status body failures, oversized responses and extraction failures do not retry. Other error-status body failures retain status-based retries. Invalid Retry-After values use bounded jittered backoff.
- Discovery retry eligibility uses typed provider state, independent of display messages. Only ordinary deadline outcomes are eligible; rate_limited_deadline, challenges, unrecognized pages, hard failures, cancellations and completed empty responses remain ineligible. Diagnostic outcome strings and error/exit behavior are unchanged; unknown report outcomes must not be treated as retryable deadlines.
...
- Bing uses its fixed Chrome 146/macOS profile; other search and fetch clients select only Firefox 146 on Windows or Chrome 146 on macOS browser headers. All reuse HTTP/2 or HTTP/1.1 connections within the process.
"#;
